...
const GuestPage = () => {
  ...
  useScanner({
    ...
    minLength: 3,
    disabled: showNumPad, // Disable scanner when numpad is open to avoid conflicts
  });
  ...
  // ── Print: block if mealDetails is empty, otherwise print one ticket per meal slot ──
  const handlePrint = async () => {
    const guest = displayGuests.find((g) => g.requestId === selectedId);
    if (!guest) return;

    // ── Guard: QR codes must be generated by backend before printing ────────
    const mealDetails = guest.guestDetails?.mealDetails ?? [];
    if (mealDetails.length === 0) {
      setError(
        "QR codes not yet generated for this guest. Please try again later.",
      );
      return;
    }
